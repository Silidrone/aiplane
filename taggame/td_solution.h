...
static const std::string Q_INPUT_FILE = "q.json";
static const std::string POLICY_INPUT_FILE = "sp.json";

// TODO: Refactor later so that it can be accessible in a more generalized context, not just here in
// taggame_main.

inline bool load_Q_from_file(std::string file_path, MDPSolver<State, Action>& mdp_solver) {
...
inline bool load_stochastic_policy_from_file(std::string file_path, StochasticPolicy<State, Action>& policy) {
    if (!std::filesystem::exists(POLICY_INPUT_FILE)) return false;
...
inline int taggame_main() {
    ...
    load_Q_from_file(output_dir + Q_INPUT_FILE, mdp_solver);
    load_stochastic_policy_from_file(output_dir + POLICY_INPUT_FILE, policy);
