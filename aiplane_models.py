...
def feature_extractor(state: AiplaneState, action: AiplaneAction, device=torch.device("cpu")) -> torch.Tensor:
    ...
    norm_msl = max(-1.0, min(1.0, 1 - 2 * (msl - 116) / (450 - 116)))
    norm_lateral = max(-1.0, min(1.0, lateral_dev / 500))
    norm_vertical = max(-1.0, min(1.0, vertical_dev / 120))
    norm_vspeed = max(-1.0, min(1.0, vertical_speed / 10))
    norm_pitch = max(-1.0, min(1.0, pitch / 35))
    norm_bank = max(-1.0, min(1.0, bank / 40))
    norm_airspeed = max(0.0, min(1.0, (airspeed - 25) / (120 - 25)))
