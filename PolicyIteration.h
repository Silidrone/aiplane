...
class PolicyIteration : public MDPSolver<State, Action> {
   ...
   public:
    PolicyIteration(MDPCore<State, Action>* mdp_core, const double discount_rate,
                    const double policy_evaluation_threshold)
        : MDPSolver<State, Action>(mdp_core),
          m_discount_rate(discount_rate),
          m_policy_evaluation_threshold(policy_evaluation_threshold) {}
